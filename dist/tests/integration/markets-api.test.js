...
describe('GET /api/markets', () => {
    ...
    it('includes the seeded ZA market', async () => {
        const res = await request.get('/api/markets');
        expect(res.status).toBe(200);
        const za = res.body.find((m) => m['code'] === 'ZA');
    ...
    `);
        const res = await request.get('/api/markets');
        expect(res.status).toBe(200);
        const codes = res.body.map((m) => m['code']);
    ...
    `);
        ...
        // cleanup
        await client_1.db.query(`DELETE FROM markets WHERE code = 'TZ'`);
    });
});
// ---------------------------------------------------------------------------
...
describe('GET /api/markets/:code', () => {
    ...
    `);
        ...
        expect(res.status).toBe(404);
        await client_1.db.query(`DELETE FROM markets WHERE code = 'YY'`);
    });
